...
imagenes.save_generated_vtk(desnorm_data, z_vals, output_folder=os.path.join(trained_models_folder, "vtk_real"), log_scale=True)
...
#for i in range(num_cv):
 #   histogramas.histo_individual(desnorm_fake_agrupados[i], desnorm_data_agrupados[i], i)
#histogramas.histo_fake(desnorm_fake_agrupados[0:100])
#histogramas.all_histogramas(N, desnorm_fake_agrupados, desnorm_data_agrupados, "desnorm", epoch)

#histogramas.histograma_gpt(N, desnorm_fake_agrupados[0:100], desnorm_data_agrupados[0:27], "desnorm", epoch)
#histogramas.comparar_histogramas_promedio2(desnorm_fake_agrupados[3300:], desnorm_data_agrupados[891:])

power.generate_psd_real(k_values, psd_mean_desnorm[33], psd_mean_desnorm[0], generated_images_folder)
