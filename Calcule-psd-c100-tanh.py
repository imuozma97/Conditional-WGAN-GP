...
data = datos.data0('Camels_data/Data3D-64.hdf5')
delta = datos.delta(data[0])
print(np.min(delta), np.max(delta))
forw = forward(delta)
norm_data, max_val, min_val = datos.normalizar_datos(forw) 
z_vals = norm_data[1].astype("float32") 
...
psd_norm_agrupado = datos.reordenacion(psd_norm, data[1])[0]
...
np.savez("PSD_norm_c100", psd = psd_norm, psd_agrupado = psd_norm_agrupado, mean = psd_mean_norm , sigma = sigma_norm,  sigma_log = sigma_log_norm, psd_max = psd_max_norm , psd_min = psd_min_norm , k_values = k_values)
