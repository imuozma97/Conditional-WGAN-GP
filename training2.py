...
class Training(tf.keras.Model):
    ...
    def __init__(self, data_class, discriminator1, discriminator2, generator, batch_size, ncritic, trained_models_folder, generated_images_folder, lambda_psd_schedule, use_psd=True, use_psd_loss = True):
        super().__init__()
        ...
        self.use_psd = use_psd  #Aqui este use_psd se refiere a si uso la rama espectral del D
    ...
    def compile(self, d_optimizer1, g_optimizer, d_optimizer2):
        super().compile()
    ...
    def train_step(self, data):
        ...
        real_images, z_values, psd_max, psd_min, sigma_log, psd_real = data  
        ...
        for _ in range(self.ncritic):
            ...
            with tf.GradientTape(persistent=True) as disc_tape:
                ...
                if self.use_psd:
                    fake_predictions = self.discriminator1([generated_images, z_values], training=True)
                    real_predictions = self.discriminator1([real_images, z_values], training=True)

                    fake_predictions_spect = self.discriminator2([psd_gen, z_values], training=True)
                    real_predictions_spect = self.discriminator2([psd_real, z_values], training=True)

                else:
                    fake_predictions = self.discriminator1([generated_images, z_values], training=True)
                    real_predictions = self.discriminator1([real_images, z_values], training=True)
                
                gp, grads_norm_mean = gradient_penalty(real_images, generated_images, z_values, self.discriminator1, self.batch_size, 10)
                ...
                disc_loss_spect_fake = tf.reduce_mean(fake_predictions_spect)
                disc_loss_spect_real = tf.reduce_mean(real_predictions_spect)
                wass_loss_spect = disc_loss_spect_fake - disc_loss_spect_real

            grads_disc = disc_tape.gradient(disc_loss, self.discriminator1.trainable_variables)
            norm_disc = tf.linalg.global_norm(grads_disc)
            self.d_optimizer1.apply_gradients(zip(grads_disc, self.discriminator1.trainable_variables))

            grads_disc_spect = disc_tape.gradient(wass_loss_spect, self.discriminator2.trainable_variables)
            norm_disc_spect = tf.linalg.global_norm(grads_disc_spect)
            self.d_optimizer2.apply_gradients(zip(grads_disc_spect, self.discriminator2.trainable_variables))
        ...
        with tf.GradientTape() as gen_tape:
            ...
            loss_spect = -tf.reduce_mean(fake_predictions_spect)
            gen_loss = loss_adv + 0.1*loss_spect

            percent = psd_out_of_band_fraction(psd_gen, psd_min, psd_max)

            loss_psd = psd_loss(psd_gen, psd_real, sigma_log) 
    ...
    def train(self, dataset_train,  epochs):
        ...
        checkpoint = tf.train.Checkpoint(generator = self.generator, discriminator = self.discriminator1, g_optimizer = self.g_optimizer, d_optimizer = self.d_optimizer, epoch = tf.Variable(0))
