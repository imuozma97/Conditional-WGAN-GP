...
from psd_utils import lambda_psd_schedule4
...
cgan = Training128(data_class = datos, discriminator = discriminator, generator = generator, batch_size = batch_size1, ncritic = ncritic3, 
                trained_models_folder = trained_models_folder, generated_images_folder = generated_images_folder, lambda_psd_schedule = lambda_psd_schedule4,
                lambda_term = 20, image_size = image_size2, use_psd = False, use_psd_loss = True)

cgan.compile(d_optimizer = tf.keras.optimizers.Adam(learning_rate = 0.000025, beta_1 = 0, beta_2 = 0.9),
             g_optimizer = tf.keras.optimizers.Adam(learning_rate = 0.00005, beta_1 = 0, beta_2 = 0.9))
