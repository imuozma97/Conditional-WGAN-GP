...
datos= Dataset(27, n_bar, buffer_size = 918)
...
# Índices a eliminar en cada bloque
eliminar = [4, 12, 25]
mask = np.ones(34, dtype=bool)
mask[eliminar] = False

# Reorganizar en bloques de 34
z_vals_reshape = z_vals.reshape(-1, 34)
z_vals_final = z_vals_reshape[:, mask].reshape(-1, 1)
...
psd_max_reshape = psd_max.reshape(-1, 34, 32)
psd_max_final = psd_max_reshape[:, mask].reshape(-1, 32)

psd_min_reshape = psd_min.reshape(-1, 34, 32)
psd_min_final = psd_min_reshape[:, mask].reshape(-1, 32)

mean_psd_reshape = mean_psd.reshape(-1, 34, 32)
mean_psd_final = mean_psd_reshape[:, mask].reshape(-1, 32)

psd_sigma_reshape = psd_sigma.reshape(-1, 34, 32)
psd_sigma_final = psd_sigma_reshape[:, mask].reshape(-1, 32)



dataset = datos.crea_dataset(cubos_final, z_vals_final, psd_max_final, psd_min_final, mean_psd_final, psd_sigma_final)
...
cgan = Training(data_class = datos, discriminator = discriminator, generator = generator, batch_size = 27, ncritic = ncritic3, 
