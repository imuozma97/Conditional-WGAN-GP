...
transf_delta = datos.asinh(delta)

norm_data, max_val, min_val = datos.normalizar_datos(transf_delta) 
print('norm_data shape', norm_data.shape)
z_vals = datos.normalizar_z(red)

k_values = power.compute_psd(np.squeeze(norm_data[0]))[1]
...
np.savez("PSD_asinh_alpha1_norm.npz", psd = psd_norm, psd_agrupado = psd_norm_agrupado, mean = psd_mean_norm , sigma = sigma_norm,  sigma_log = sigma_log_norm, psd_max = psd_max_norm , psd_min = psd_min_norm , k_values = k_values)
